...
public:
	...
	GoToSelectedBall() : ac("move_base", true) {
...
//	ac.waitForResult();
	...
	//			goToSelectedBall.offHoover();
		...
		//	automat związany z dojezaniem do pileczek na ostatniej prostej
		if(goToSelectedBall.alg_state_ == GO_FORWARD_INTRO){

			if(goToSelectedBall.moveStraightState == 1 && goToSelectedBall.moveStraightStateChange == true){
					// jedzie do piłeczki, nic nie robimy
				ROS_INFO("state GO_FORWARD_INTRO");
				continue;
			}
			else if(goToSelectedBall.moveStraightState == 2 && goToSelectedBall.moveStraightStateChange == true){
				//	dojechal, zmieniamy stan na nastepny
				goToSelectedBall.alg_state_ = GO_FORWARD_COLLECT;
				ROS_INFO("change state GO_FORWARD_INTRO -> GO_FORWARD_COLLECT");
				goToSelectedBall.onHoover();
				goToSelectedBall.goForward(0.2);
				continue;
			}
			...
			if(goToSelectedBall.moveStraightState == 1 && goToSelectedBall.moveStraightStateChange == true){
					// jedzie do piłeczki, nic nie robimy
				ROS_INFO("state GO_FORWARD_COLLECT");
				continue;
			}
			else if(goToSelectedBall.moveStraightState == 2 && goToSelectedBall.moveStraightStateChange == true){
				//	dojechal, zmieniamy stan na nastepny
				goToSelectedBall.alg_state_ = GO_FORWARD_FINISH;
				ROS_INFO("change state GO_FORWARD_COLLECT -> GO_FORWARD_FINISH");
				goToSelectedBall.onHoover();
				goToSelectedBall.goForward(0.4);
				continue;
			}
			...
			if(goToSelectedBall.moveStraightState == 1 && goToSelectedBall.moveStraightStateChange == true){
					// jedzie do piłeczki, nic nie robimy
				ROS_INFO("state GO_FORWARD_FINISH");
				continue;
			}
			else if(goToSelectedBall.moveStraightState == 2 && goToSelectedBall.moveStraightStateChange == true){
				//	dojechal, zmieniamy stan na nastepny
				ROS_INFO("change state GO_FORWARD_FINISH -> LOOKING_FOR_BALLS");
				goToSelectedBall.alg_state_ = LOOKING_FOR_BALLS;
				goToSelectedBall.offHoover();
				continue;
			}
		}
		else if(goToSelectedBall.alg_state_ == LOOKING_FOR_BALLS){
			ROS_INFO("state LOOKING_FOR_BALLS");

			if(goToSelectedBall.isBallPoseSet== true){
				//	zmiana stanu na GO_TO_BALL_WITH_NAV
				ROS_INFO("change state LOOKING_FOR_BALLS -> GO_TO_BALL_WITH_NAV");
				goToSelectedBall.alg_state_ = GO_TO_BALL_WITH_NAV;
				continue;
			}
			else{
				//	eksploracja
		...
		else if(goToSelectedBall.alg_state_ == GO_TO_BALL_WITH_NAV){

			ROS_INFO("state GO_TO_BALL_WITH_NAV");

			if(goToSelectedBall.getDistanceFromSelectedBall() > 0.7){
				goToSelectedBall.offHoover();

				float angleDiffRobotGoal = goToSelectedBall.getAngleDiff()*180/(3.14);
				if(angleDiffRobotGoal > 5.0){
					goToSelectedBall.publishAngle();
					goToSelectedBall.ac.waitForResult();
				}
				else{
				...
				goToSelectedBall.ac.cancelAllGoals ();
				goToSelectedBall.goForward(0.2);
				ROS_INFO("change state GO_TO_BALL_WITH_NAV -> GO_FORWARD_INTRO");
...
/*
	while (ros::ok()) {
		...
		while (ros::ok()) {
			...
			if(goToSelectedBall.isBallPoseSet== false){
				ROS_INFO("publishPose, wait for ball position");
				continue;
			}

			if(	firstGoalSent == true && goToSelectedBall.ac.getState().isDone() == false){
				continue;
			}
		...
		while (ros::ok()) {
			...
			goToSelectedBall.publishPose(goToSelectedBall.getCurrentPose().x, goToSelectedBall.getCurrentPose().y, 0.2);
			break;
		}
	}
